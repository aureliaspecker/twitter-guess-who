...
def setup():
    ...
    try:
        ...
        return render_template(
            "setup.html", form=form, next_page="/play", num_users=tgw.num_users
        )
    except:
        return redirect("/error")
...
def fetch_data():
    ...
    try:
        ...
        return render_template(
            "play.html", gif_url=gif_url, next_page=f"/round{tgw.next_round}"
        )
    except:
        return redirect("/error")
    ...
    try:
        ...
        if form_list.is_submitted():
            ...
            for i in range(num_users):
                if (
                    sorted_tweets[i]
                    == tweet_counts[int(form_list.select_forms.data[i]["select"])]
                ):
                    points += 1
        ...
        return render_template(
            "round1.html", n=num_users, form_list=form_list, tweet_counts=sorted_tweets
        )
    except:
        return redirect("/error")
...
def round2():
    ...
    try:
        ...
        if form_list.is_submitted():
            ...
            return redirect("/score")
        return render_template(
            "round2.html", n=num_users, form_list=form_list, user_bios=user_bios
        )
    except:
        return redirect("/error")
...
def round3():
    ...
    try:
        ...
        if form_list.is_submitted():
            points = 0
            for i in range(num_users):
                player_answer = wordcloud_paths[
                    int(form_list.select_forms.data[i]["select"])
                ]
            ...
            return redirect("/score")
        return render_template(
            "round3.html", n=num_users, form_list=form_list, wc_paths=wordcloud_shuffle
        )
    except:
        return redirect("/error")
...
def score():
    ...
    try:
        ...
        try:
            ...
            gif_url = json.loads(gifs.text)["data"][random_gif_id]["images"][
                "fixed_height"
            ]["url"]
        except:
        ...
        return render_template(
            ...
            score=score,
            max_score=max_score,
            next_page=f"/round{tgw.next_round}",
            gif_url=gif_url,
        )
    except:
        return redirect("/error")
...
def goodbye():
    ...
    try:
        ...
        gif_url = json.loads(search_gif(query=gif_tags).text)["data"][0]["images"][
            "fixed_height"
        ]["url"]
        return render_template(
            "goodbye.html", gif_url=gif_url, next_page="/index", response=response
        )
    except:
        return redirect("/error")
